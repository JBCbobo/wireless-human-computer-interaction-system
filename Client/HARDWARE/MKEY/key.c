...
{	
	...
	'D',  '6',  '5',  '4',     //'D'表示delete     回删键
	
	'E',  '3',  '2',  '1',     //'E'表示eliminate  清除键
	
	'Y',  'C',  '.',  '0'      //'C'表示cancle     取消键
};
...
static void GPIO_Configuration1(void)
{
	...
	GPIOB->CRH |= 0X33333333;//最大输出速度50M
...
uint8_t scan_MatrixKey(void)
{
	...
	GPIO_Configuration1();
	tmp = PORT;
	if((PORT&0XF000)!=0XF000)//如果按键按下
	{
        delay_us(100);
		tmp = PORT;
		if((PORT&0XF000)!=0XF000)//延时消抖
		{   	
	        column = tmp & 0XF000;//获取列号 
	        GPIO_Configuration2();
	        row = PORT & 0X0F00;//获取行号                                         
	        switch ((uint8_t)((column | row)>>8))//column|row为按键被按下对应端口的编码 注意IO使用的是高八位还是低八位。
	        {   
	            //按键对应的码表
